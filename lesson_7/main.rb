...
class Main
  ACTIONS = {
    1 => :create_station,
    2 => :create_train,
    3 => :add_carriage_to_train,
    4 => :remove_carriage_from_train,
    5 => :add_train_to_station,
    6 => :display_station_trains,
    7 => :display_train_carriages,
    8 => :take_place_in_carriage
  }.freeze
    ...
    if needed_train[0].nil?
      ...
      @all_trains[index_of_train].each do |carriage|
        puts "##{carriage.number}
          type - #{carriage.type}
          free places - #{carriage.free_places}
          taken places - #{carriage.taken_places}
          -----------------------"
      end
    end
  end
